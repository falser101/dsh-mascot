...
- **状态气泡**：忙碌时（思考/调工具/写答案/出错等）气泡持续显示当前状态；**闲置时气泡按三档频率自动弹出**——安静（60 秒/次）、标准（40 秒/次，默认）、热闹（20 秒/次），每次显示 6-8 秒后隐去，文案按内置池+AI 池轮换；悬停随时可看。可在设置页调整频率或关闭（关闭后仅悬停显示）。
- **AI 小剧场**：闲置文案池里**每 5 句插入 1 句由框架已配置模型（`agent-default-model`）现场生成的俏皮话**——调用宿主 `GET /mascot/lines` 路由，一批生成 6 条、10 分钟才真正调一次模型（成本极低），失败自动降级纯内置轮换。可在设置页关闭。
...
- **双击**：收起成小圆点 / 唤出。
- **形象切换**：设置页 → 通用 → 「悬浮伙伴形象」选择猫咪/狗狗/**自定义形象**（AI 生成图），选择持久化。
